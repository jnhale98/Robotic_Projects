...
function cspace = ex2_cspace(questionNum, cspace)
    ...
    q_goal = [3.05; 0.05];
    if nargin < 2
        % Need to set to 300 instead of 100 to complete C7 without covering
        % start configuration with obstacles in cspace
    ...
    if questionNum == 3
        ...
        % compute the configuration spac[q1val, q1_loc] = min(q1_check);e using C2
        if nargin < 2
            cspace = C2(robot, obstacles, q_grid);
        end
